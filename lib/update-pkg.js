...
module.exports = (
  ...
  return {
    ...
    types: when(useTS, 'dist/index.d.ts'),
    files: when(
      compile,
      ['dist'],
      ['src/**']
    ...
    scripts: {
      ...
      build: when(compile, 'rollup -c && tsc --emitDeclarationOnly'),
    ...
    'lint-staged': {
      '*.{js,ts}': when(useLinter, [`${eslint} --fix`, 'git add']),
      [when(useLinter, '*.{json,md}', '*.{js,ts,json,md}')]: [
        'prettier --write',
        'git add'
      ]
    }
